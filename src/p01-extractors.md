...
"Принципы функционального программирования в Scala", что проходит
...
Вполне возможно, вы -- один из них, или ваше знакомство со Scala 
...
Где это будет уместно, я попытаюсь показать примеры идиоматичного^{TM} 
применения Scala.

Но для вступления -- достаточно. При чтении помните, 
...
мы скоро познакомимся. Для Case-классов Scala автоматически создаёт ???объект-компаньон???
(*companion object*): сиглтон, который содержит не только метод-конструктор `apply`, но и 
метод-экстрактор `unapply` -- именно с помощью этого метода происходит разбор значения
...
Запомните, что тип `Option` в Scala -- это безопасная альтернатива нулевым указателям (`null`). Мы посвятим 
...
`Option` типа `TupleN`, где `N` -- число извлекаемых параметров. 
...
записи для экстракторов. Так, вместо `e(p1, p2)`, где `e` -- экстрактор и `p1` 
и `p2` -- извлекаемые параметры, мы всегда можем написать `p1 e p2`.
...
Несмотря на то, что в том как мы воспользовались оператором `#::` при сопоставлении с образцом 
нет ничего особенного, давайте присмотримся к нему по-внимательней. Также в этом экстракторе 
нам встретится метод `unapply`, который в зависимости от структуры аргумента может возвращть `None`. 
...
пройдёт сопоставление с потоком, состоящим из одного и более элементов. Если потоке содержит
...
`case`-классов -- очень хорошая практика. Они применяются для построения 

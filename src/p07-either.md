...
Мы узнали о типе `Try`. Он появился в Scala с версии 2.10. Также я упомянул и о типе `Either`.
В этой главе мы остановимся на нём поподробнее. Мы узнаем, как и где он используется, и о некоторых
...
Что бы ни содержало значение `Either[String, Source]` в этом примере, `Left` или `Right`, оно будет
...
// moreContent содержит Left с msg, полученным из getContent в Iterator'е
~~~
...
равнозначными. Но Вы уже наверное, догадываетесь, какие проблемы могут стоять за этим
...
для работы с различными типами. Также мы можем использовать `for`-генераторы для проекций,

...
Мы не можем просто присвоить полю переменной `a2` значение `B`, которое связанное с `a1`. Она
ожидает значения типа `a2.B`, а мы передаём `a1.B`. Синтаксис вызова через точку представляет 
